...
func generateFontC(_ glyphsByLanguage: [(Language, [String])]) -> String {
    ...
    for (lang, glyphs) in glyphsByLanguage {
        ...
        out += "static const uint8_t \(lang.glyphArray)[][DISPLAY_GLYPH_BYTES] = {\n"
        for glyph in glyphs {
        ...
        out += "static const uint8_t \(lang.glyphArray)_2x[][DISPLAY_GLYPH_2X_BYTES] = {\n"
        for glyph in glyphs {
        ...
        out += """
        const display_glyph_font_t \(lang.fontObject) = {
...
try generateI18nInc(glyphsByLanguage)
    .write(to: root.appendingPathComponent("main/i18n_gen.inc"), atomically: true, encoding: .utf8)
