...
// draw_line_cached). Rows with non-ASCII text use pixel-width centering.
...
static bool clock_text_has_non_ascii(const char *s) {
    while (*s) {
...
// cell-by-cell against what is on the glass. Non-ASCII strings cannot use
...
static void draw_line_cached(int y, char *cache, size_t cache_len,
    ...
    bool line_non_ascii = clock_text_has_non_ascii(line);
    if (line_non_ascii || clock_text_has_non_ascii(cache)) {
        ...
        if (!line_non_ascii) {
            // The row went pure-ASCII: erase the pixel-centered rendering
            // and rebuild below with an empty cache, so the grid path never
            // diffs against a raw pixel-path cache (the two cache formats
